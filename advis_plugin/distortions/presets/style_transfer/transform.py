...
class Transform:
	...
	def _conv_layer(self, net, num_filters, filter_size, strides, \
	...
	def _residual_block(self, net, filter_size=3, name=None):
		...
		tmp = self._conv_layer(net, 128, filter_size, 1, padding='VALID', relu=True,
			name=name+'_1')
		return self._conv_layer(tmp, 128, filter_size, 1, padding='VALID',
			relu=False, name=name+'_2') + tf.slice(net, [0,2,2,0],
			[batch,rows-4,cols-4,channels])
